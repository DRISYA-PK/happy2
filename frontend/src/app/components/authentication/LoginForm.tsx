...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(setCredentials({ user, isAdmin: false }));
      ...
      if (error instanceof TypeError && error.message.includes("fetch")) {
        if (username === "user" && password === "user") {
          ...
          const demoUser = { id: 100, username: "user", name: "Jane Doe" };
          dispatch(setCredentials({ user: demoUser, isAdmin: false }));
        ...
        // Support admin bypass here too for convenience
        if (username === "admin" && password === "admin") {
          setIsSuccess(true);
          toast.success("Demo Mode Admin Login successful!", {
            description: "Bypassing server connection.",
          });
    ...
    >
      ...
      <div className="flex flex-col items-center text-center mb-7 relative z-10">
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-[280px] leading-relaxed">
          Sign in to your member account to access programs and resources.
        </p>
      </div>
